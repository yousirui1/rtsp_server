...
CHAR *get_hostname()
{
	...
	if(getdomainname(rtsp[0]->host_name + l + 1, sizeof(rtsp[0]->host_name) - 1) != 0)
	{
		rtsp[0]->host_name[l] = '.';
	}
...
S32 get_describe_sdp(CHAR *sdp_buff, S32 cur_conn_num)
{
	...
	strcpy(sdp_buff, "o=");
	strcpy(sdp_buff, get_SDP_user_name(s));
	strcpy(sdp_buff, " ");
	strcpy(sdp_buff, get_SDP_session_id(s));
	strcpy(sdp_buff, " ");
	strcpy(sdp_buff, get_SDP_version(s));
	strcpy(sdp_buff, SDP_EL);
	strcpy(sdp_buff, "c=");
	strcpy(sdp_buff, "IN "); 		//Network type :Internal
	strcpy(sdp_buff, "IP4 ");		//Address type IP4
	strcpy(sdp_buff, rtsp[0]->host_name);
	strcpy(sdp_buff, SDP_EL);
	strcpy(sdp_buff, "s=RTSP Session"SDP_EL);
	...
	strcpy(sdp_buff, "t=0 0"SDP_EL);
	//media specific
	strcpy(sdp_buff, "m=");
	strcpy(sdp_buff, "vide 0");
	strcpy(sdp_buff, "RTP/AVP");  //Use UDP
